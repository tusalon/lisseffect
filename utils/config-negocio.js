...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '07cc818e-1781-45b5-9f5a-25e53b1adb95'; // ID de liss_effect
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'liss_effect';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53290988';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'stitchvega801@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a liss_effect!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'lisseffect';
};
...
console.log('✅ config-negocio.js listo para liss_effect');
